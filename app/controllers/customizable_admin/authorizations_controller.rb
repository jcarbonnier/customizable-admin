module CustomizableAdmin
  class AuthorizationsController < CustomizableAdmin::ApplicationController
    ...
    #-----
    def index(opts = {})
      # instance_variable_set("#{@current_item_name.to_s.pluralize}".to_sym, @items)
      # @current_model.column_names.each { |attr| @default_attrs[attr] = nil if ((!attr.blank?) and attr.index('_attributes').nil?()) }
      # @default_attrs.merge!(params[@current_item_name]) if (params[@current_item_name])
      # Rails.logger.info("Default attributes: #{@default_attrs.to_yaml}")
      @default_attrs ||= {}
    ...
    def show(opts = {})
      # instance_variable_set("#{@current_item_name}".to_sym, @item)
    ...
    def create(opts = {})
      # @item = @current_model.new(resource_params)
      # instance_variable_set("#{@current_item_name}".to_sym, @item)
      @item.save
      respond_with(@item) unless (opts[:skip_response])
    end
    ...
    def initialize_application(opts = {})
      ...
      # Default options merged with parameters opts
    ...
    def configure_permitted_parameters
      Rails.logger.debug("configure_permitted_parameters")
    ...
    def sort_column
      ...
      # Rails.logger.debug("Sort column : #{t_name}, #{c_name}")
